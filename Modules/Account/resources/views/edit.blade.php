    ...
    <form class="flex flex-col p-6" action="{{ route('account.update', ['account' => $user->login_id]) }}" method="POST">
      ...
      <div class="grid gap-4 p-2">
        <label class="flex flex-col gap-2">
          ログインID（社員番号も可）
          <input class="rounded-lg border border-slate-300 px-3 py-2 font-normal" name="loginId" type="text"
            value="{{ old('loginId', $user->login_id) }}" required>
          @error('loginId')
      ...
      <div class="grid gap-4 p-2">
        ...
        <label class="flex flex-col gap-2">
          名前（フリガナ）
          <input class="rounded-lg border border-slate-300 px-3 py-2 font-normal" name="nameKana" type="text"
            value="{{ old('nameKana', $user->profile?->name_kana) }}">
          @error('nameKana')
            <div class="font-normal text-red-500">{{ $message }}</div>
          @enderror
        </label>
        <label class="flex flex-col gap-2">
          郵便番号
          <input class="rounded-lg border border-slate-300 px-3 py-2 font-normal" name="postCode" type="text"
            value="{{ old('postCode', $user->profile?->post_code) }}">
          @error('postCode')
            <div class="font-normal text-red-500">{{ $message }}</div>
          @enderror
        </label>
        ...
        <label class="flex flex-col gap-2">
          電話番号
          <input class="rounded-lg border border-slate-300 px-3 py-2 font-normal" name="phoneNumber" type="text"
            value="{{ old('phoneNumber', $user->profile?->phone_number) }}">
          @error('phoneNumber')
            <div class="font-normal text-red-500">{{ $message }}</div>
          @enderror
        </label>
        ...
        <label class="flex flex-col gap-2">
          入社日
          <input class="js-datepicker rounded-lg border border-slate-300 px-3 py-2 font-normal" name="hireDate"
            type="text" value="{{ old('hireDate', $user->profile?->hire_date) }}">
          @error('hireDate')
            <div class="font-normal text-red-500">{{ $message }}</div>
          @enderror
        </label>
      ...
      <div class="grid gap-4 p-2">
        <label class="flex flex-col gap-2">
          緊急連絡先　氏名
          <input class="rounded-lg border border-slate-300 px-3 py-2 font-normal" name="emergencyName" type="text"
            value="{{ old('emergencyName', $user->profile?->emergency_name) }}">
          @error('emergencyName')
            <div class="font-normal text-red-500">{{ $message }}</div>
          @enderror
        </label>
        <label class="flex flex-col gap-2">
          緊急連絡先
          <input class="rounded-lg border border-slate-300 px-3 py-2 font-normal" name="emergencyPhoneNumber"
            type="text" value="{{ old('emergencyPhoneNumber', $user->profile?->emergency_phone_number) }}">
        ...
        <label class="flex flex-col gap-2">
          緊急連絡先の続柄
          <input class="rounded-lg border border-slate-300 px-3 py-2 font-normal" name="emergencyRelationship"
            type="text" value="{{ old('emergencyRelationship', $user->profile?->emergency_relationship) }}">
