<x-dashboard-layout>
  <x-dashboard.index>
    ...
    <x-dashboard.container>
      ...
      <div class="mt-[30px] hidden grid-cols-[8%,20%,20%,16%,14%,14%,4%,4%] sm:grid">
        ...
        <div class="text-left text-xs font-normal text-[#AAB0B6]">ログインID</div>
        <div class="text-left text-xs font-normal text-[#AAB0B6]">契約区分</div>
        <div class="text-left text-xs font-normal text-[#AAB0B6]">更新日</div>
        <div class="text-left text-xs font-normal text-[#AAB0B6]">最終ログイン日</div>
      ...
      <div class="mt-[24px] rounded-lg border-b sm:-mx-0 sm:mt-[8px] sm:border">
        @foreach ($users as $user)
          <div @class([
              'sm:grid sm:grid-cols-[8%,20%,20%,16%,14%,14%,4%,4%] sm:py-[18px] py-3 text-[15px] sm:px-0 px-5 cursor-pointer items-center',
              'border-b' => !$loop->last,
          ])>
            ...
            <div class="text-[15px]">{{ $user->login_id }}</div>

            <div class="text-[15px]">{{ $user->profile?->contract_type }}</div>

            <div class="text-[15px]">{{ $user->profile?->updated_at->format('Y年m月d日') }}</div>
            ...
            <a href="{{ route('account.edit', ['account' => $user->login_id]) }}"><svg width="24" height="24"
