<x-modal name="group-edit-modal-{{ $group->id }}" maxWidth="440" mobileMaxWidth="350">
  <form wire:submit="update" enctype="multipart/form-data">
    ...
    <div>
      ...
      <button
        ...
        type="button" onclick="document.getElementById('icon{{ $group->id }}').click()">
      ...
      <x-text-input class="hidden" id="icon{{ $group->id }}" type="file" accept="image/*"
    ...
    @elseif($form->group->icon)
      <div class="mt-3 flex items-center justify-between">
        <div class="flex space-x-2">
          <img class="h-[20px] w-[20px] rounded object-cover" src="{{ $form->group->icon }}" alt="プレビュー">
          <p class="truncate text-sm">
            {{ basename($form->group->icon) }}
          </p>
        </div>
      </div>
    @else
    ...
    <div class="mt-5">
      ...
      <x-text-input class="block w-full" type="text" placeholder="グループ名" wire:model="form.name" required />
    </div>
      ...
      <div class="flex flex-col gap-2 text-black">
        ...
        <div class="grid grid-cols-3 gap-3 lg:grid-cols-4">
          @foreach ($this->users as $user)
            <label class="block cursor-pointer rounded border p-2 transition" x-data="{ checked: @js(in_array($user->id, $form->member ?? [])) }"
              :class="checked ? 'bg-[#3289FA] text-white' : 'bg-white'">
              <input class="hidden" type="checkbox" value="{{ $user->id }}" wire:model="form.member"
                x-model="checked">
              {{ $user->name }}
            </label>
          @endforeach
