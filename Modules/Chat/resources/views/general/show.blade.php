<x-app-layout :url="route('chat.index')">
  <div class="flex w-full justify-between" x-data="{ viewGroupCreateDialog: false }" x-cloak>
    <div class="hidden h-screen w-full shrink-0 overflow-y-auto bg-white px-[15px] py-[30px] lg:block lg:w-[280px]">
        ...
        <div class="border-b px-5 py-[10px]">
          <div class="grid grid-cols-[35px,auto,auto] lg:grid-cols-[50px,auto,200px]" x-data="{ showMemberList: false }">
            @if ($selectGroup->is_dm)
              ...
              @else
                <div
                  class="flex h-[30px] w-[30px] items-center justify-center rounded-full border bg-white lg:h-10 lg:w-10">
            ...
            @else
              @if ($selectGroup->IconImage)
                <img class="h-[30px] w-[30px] rounded-full border bg-white lg:h-10 lg:w-10"
                  src="{{ $selectGroup->IconImage }}" alt="アイコン">
              @else
                <div
                  class="flex h-[30px] w-[30px] items-center justify-center rounded-full border bg-white lg:h-10 lg:w-10">
                  <i class="fa-solid fa-image"></i>
