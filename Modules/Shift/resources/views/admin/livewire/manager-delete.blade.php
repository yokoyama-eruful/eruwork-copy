<div>
  <button class="hover:opacity-40" type="button"
    x-on:click="$dispatch('open-modal', 'delete-modal-{{ $manager->id }}')"><svg width="20" height="20"
      ...
      <path
        ...
        stroke="#FF4A62" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
    </svg>
  </button>
  <x-modal-alert name="delete-modal-{{ $manager->id }}" title="削除" maxWidth="sm">
    ...
    <div class="my-5 flex items-center justify-center space-x-[10px]">
      <div class="flex h-11 w-[150px] cursor-pointer items-center justify-center rounded border-2"
        @click="$dispatch('close-modal', 'delete-modal-{{ $manager->id }}')">キャンセル</div>
      <button class="flex h-11 w-[150px] cursor-pointer items-center justify-center rounded bg-[#FF4A62] text-white"
        type="button" wire:click="delete({{ $manager->id }})">削除する</button>
    </div>
  </x-modal-alert>
</div>
