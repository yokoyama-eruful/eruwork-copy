<div class="relative" x-data="{ breakTimeModal{{ $breakTime->id }}: false }">
  ...
  <div class="absolute right-0 top-7 z-10 w-[95px] rounded-xl bg-white px-4 py-2 shadow-md"
    x-show="breakTimeModal{{ $breakTime->id }}==true" x-on:click.away="breakTimeModal{{ $breakTime->id }}=false">
    ...
    <button class="flex items-center pt-[11px]" type="button"
      x-on:click="$dispatch('open-modal', 'delete-break-time-modal-{{ $breakTime->id }}')">
      <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path
  ...
  <x-modal-alert name="delete-break-time-modal-{{ $breakTime->id }}" title="削除" maxWidth="sm">
    <div class="flex flex-col items-center bg-[#F7F7F7] px-5 pb-8 pt-4 text-left">
      ...
      <div class="pt-[13px] text-[0.9375rem] font-bold">
        {{ $breakTime->in_time?->format('Y-m-d H:i') }}~{{ $breakTime->out_time?->format('Y-m-d H:i') }}</div>
    </div>
    <div class="my-5 flex flex-col gap-[10px] sm:flex-row sm:items-center sm:justify-center sm:space-x-[10px] sm:gap-0">
      <div class="flex h-11 w-full sm:w-[150px] whitespace-nowrap cursor-pointer items-center justify-center rounded border-2"
        @click="$dispatch('close-modal', 'delete-break-time-modal-{{ $breakTime->id }}')">キャンセル</div>
      <div class="flex h-11 w-full sm:w-[150px] whitespace-nowrap cursor-pointer items-center justify-center rounded bg-[#FF4A62] text-white"
        wire:click="deleteBreakTime">削除する</div>
    </div>
  </x-modal-alert>

  <x-modal name="edit-break-time-modal-{{ $breakTime->id }}" :title="'休憩時間修正'">
    <form wire:submit="updateBreakTime">
