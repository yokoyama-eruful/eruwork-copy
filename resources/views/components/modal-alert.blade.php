...
<div class="fixed inset-0 z-50 overflow-y-auto px-4 py-6 sm:flex sm:items-center sm:justify-center sm:px-0"
  ...
  <div
    class="{{ $maxWidth }} mb-6 transform overflow-hidden rounded-lg bg-white p-4 shadow-xl transition-all sm:w-full"
    x-show="show" x-transition:enter="ease-out duration-300"
    x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
    x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200"
    x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100"
    x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95">
    <div class="text-right hover:text-gray-500" x-on:click="show = false"><i class="fa-solid fa-xmark"></i></div>
    <div>
      {{ $slot }}
    </div>
  </div>
