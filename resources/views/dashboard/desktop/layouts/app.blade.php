...
<body class="m-0 grid h-screen w-screen grid-rows-[50px,1fr] p-0 lg:block" x-data="{ sideMenu: false }" x-cloak>
  ...
  <div class="hidden lg:block">
    @include('dashboard.desktop.layouts.sidemenu')
  </div>
  ...
  <x-modal-alert name="logout" title="ログアウト">
    <form method="POST" action="{{ route('logout') }}">
      ...
      <div class="my-5 flex flex-col gap-[10px] sm:flex-row sm:items-center sm:justify-center sm:space-x-[10px] sm:gap-0">
        <div class="flex h-11 w-full sm:w-[150px] cursor-pointer items-center justify-center rounded border-2"
          @click="$dispatch('close-modal', 'logout')">キャンセル</div>
        <button class="flex h-11 w-full sm:w-[150px] items-center justify-center rounded bg-[#FF4A62] text-white" type="submit">
          ログアウト
        </button>
  ...
  <div class="h-[calc(var(--vh)*100)] overflow-y-auto lg:ml-[240px] lg:flex lg:w-[calc(100%-240px)]">
    {{ $slot }}
  </div>
  @livewireScripts
</body>
